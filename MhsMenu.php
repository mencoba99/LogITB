<?php

?>
<h3>Menu</h3>
<h4>Umum</h4>
<div id="cssmenu">
    <ul>
        <li><a href='DBMhs.php'><span>Dashboard Mahasiswa</span></a>
        </li>
        <li class="has-sub"><a href="ViewProfil.php"><span >Kelola Akun</span></a>
            <ul>
                <li>
                    <a href="EditMahasiswa.php">Ubah Data Diri</a>
                </li>
                <li>
                    <a href="EditUser.php">Ubah Password</a>
                </li>
            </ul>
        </li>
        <li>
            <a href="ViewPengambilanMKMahasiswa.php">Pengambilan Mata Kuliah</a>
        </li>
         <li>
            <a href="ViewWaliMahasiswa.php">Dosen Wali</a>
        </li>
    </ul>
</div>
<!--<h4>Kerja Praktek</h4>
<div id="cssmenu">
    <ul>
        <li>
            <a href="ViewKelompokKPMahasiswa.php">Kelompok KP</a>
        </li>
        <li>
            <a href="ViewSuratPengantarKPMahasiswa.php">Surat Pengantar KP</a>
        </li>
        <li>
            <a href="ViewSKBimbinganKPMahasiswa.php">SK Bimbingan KP</a>
        </li>
        <li class="has-sub"><a href='ViewBimbinganKPMahasiswa.php'><span>Bimbingan KP</span></a>
            <ul>
                <li>
                    <a href="AddBimbinganKPMahasiswa.php">Tambah Laporan Bimbingan KP </a>
                </li>
                 <li>
                    <a href="ViewBimbinganKPMahasiswa.php">Lihat Laporan Bimbingan KP</a>
                </li>
            </ul>
        </li>
        <li>
            <a href="ViewSeminarKPMahasiswa.php">Seminar KP</a>
        </li>
        <li>
            <a href="ViewNilaiDetailKPMahasiswa.php">Nilai Detail KP</a>
        </li>
    </ul>
</div>-->
<h4>Tugas Akhir</h4>
<div id="cssmenu">
    <ul>
        <li>
            <a href="ViewSKBimbinganTAMahasiswa.php">SK Bimbingan TA</a>
        </li>
        <li class="has-sub"><a href='ViewBimbinganTAMahasiswa.php'><span>Bimbingan TA</span></a>
            <ul>
                <li>
                    <a href="AddBimbinganTAMahasiswa.php">Tambah Laporan Bimbingan TA </a>
                </li>
                 <li>
                    <a href="ViewBimbinganTAMahasiswa.php">Lihat Laporan Bimbingan TA</a>
                </li>
            </ul>
        </li>
        <li>
            <a href="ViewSyaratSeminarTA1Mahasiswa.php">Syarat Seminar TA 1</a>
        </li>
        <li>
            <a href="ViewSeminarTA1Mahasiswa.php">Semninar TA 1</a>
        </li>
        <li>
            <a href="ViewDadtarHadirSeminarTA1Mahasiswa.php">Daftar Hadir Seminar TA 1</a>
        </li>
        <li>
            <a href="ViewNilaiTA1.php">Nilai TA 1</a>
        </li>
        <li>
            <a href="ViewSeminarTA2Mahasiswa.php">Seminar TA 2</a>
        </li>
        <li>
            <a href="ViewSKSidangTAMahasiswa.php">SK Sidang TA</a>
        </li>
        <li>
            <a href="ViewSidangTA2Mahasiswa.php">Sidang TA 2</a>
        </li>
        <li>
            <a href="ViewNilaiDetailTA2Mahasiswa.php">Nilai Detail TA 2</a>
        </li>
    </ul>
</div>
<!--<h4>SKPI</h4>
<div id="cssmenu">
    <ul>
        <li>
            <a href="ViewPrestasiSKPIMahasiswa.php">Prestasi</a>
        </li>
        <li>
            <a href="ViewSKPIMahasiswa.php">SKPI</a>
        </li>
    </ul>
</div>-->

